...
from app.core.db import BaseModel, transaction
...
async def load_data(
    ...
    *,
    ...
    async with transaction(store.db) as session:
        if clear_before:
...
async def main() -> None:
    ...
    import_modules(db_files, start_dir, store.logger)

    model_map = get_model_classes()

    if args.command == "dump":
        models: list[str] | None = args.models or None
        await dump_data(store, model_map, args.file_path, models)
    elif args.command == "load":
        await load_data(store, model_map, args.file_path, clear_before=args.clear)
    else:
        store.logger.info("Команда не указана. Используйте --help для справки")
        parser.print_help()
